...
import { MeetingSessionType, Origin } from '@zunou-graphql/core/graphql'
...
export const ViewAllMeetings = ({
  ...
  const filteredMeetings = useMemo(() => {
    return filterBySearchQuery(
      meetingSessionsData?.meetingSessions ?? [],
    ).filter((session) => session.type === MeetingSessionType.Meeting)
  ...
  return (
    <Stack height="100%" pt={1} spacing={2}>
      ...
      />
      {error ? (
        <Typography>Error loading meetings</Typography>
