...
class DocumentParsingHelper
{
     ...
     */
    public static $allowed_tags = null;
    ...
    public static function parseFromFile($filename, $mimetype = null)
    {
        if (! is_readable($filename)) {
            throw new \Exception(
                "Failed to read file: cannot read file: $filename",
            );
        }
        ...
        if (preg_match('/^text\/*/', $mimetype)) {
            ...
            throw new \Exception(
                "Failed to read file: unknown mimetype: $mimetype",
            );
        }
    }
    ...
    private static function parseZipped($filename, $datafile)
    {
        ...
        if ($zip->open($filename)) {
            ...
            if (($index = $zip->locateName($datafile)) !== false) {
                ...
                Log::debug('parseZipped: Content: ' . $content);
            }
        ...
        Log::debug('parseZipped (before strip_tags): Content: ' . $content);

        $stripped = strip_tags($content, self::$allowed_tags);

        $pattern     = '/(?<=。)-?\d{14,}/';
        $cleanedText = preg_replace($pattern, '', $stripped);

        Log::debug('parseZipped (after strip_tags): Content: ' . $cleanedText);
    ...
    private static function convertWordToHtml(\DOMDocument $xmldoc)
    {
        ...
        foreach ($xpath->query('//r') as $node) {
            ...
            if ($stylenodes && $textnode && $stylenodes->length) {
                ...
                $toremove = array_merge(
                    iterator_to_array($itags),
                    iterator_to_array($btags),
                );
    ...
    private static function removeDomNamespace(\DOMDocument $xmldoc, $namespace)
    {
        $xpath = new \DOMXPath($xmldoc);
        $nodes = $xpath->query(
            "//*[namespace::{$namespace} and not(../namespace::{$namespace})]",
        );
    ...
    private static function parseDoc($filename)
    {
        if (shell_exec('which antiword')) {
            ...
            $outtext = self::parseDocAntiword(
                shell_exec('antiword -x db ' . escapeshellarg($filename)),
            );
        } else {
            ...
            foreach ($paragraphs as $thisparagraph) {
                ...
                if (
                    strpos($thisparagraph, chr(0x00)) === false && strlen($thisparagraph) !== 0
                ) {
        ...
        for ($i = 0; $i < strlen($text); $i++) {
            ...
            switch ($c) {
                ...
                case '\\':
                    ...
                    if ($nc == '\\' && $isplaintext) {
                        $document .= '\\';
                    } elseif ($nc == '~' && $isplaintext) {
                        $document .= ' ';
                    } elseif ($nc == '_' && $isplaintext) {
                        $document .= '-';
                    } elseif ($nc == '*') {
                    ...
                    elseif ($nc == "'") {
                        ...
                        switch (strtolower($word)) {
                            ...
                            default:
                                $stack[$j][strtolower($word)] = empty($param)
                                    ? true
                                    : $param;
                                break;
                        }
